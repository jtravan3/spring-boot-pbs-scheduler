...
@EnableWebMvc
public class PredictionBasedPrototypeApplication implements CommandLineRunner {

    private static final List<Employee> employeeList = Stream.of(
            new Employee("1000", "Peter", 23000L),
    ...
    @Autowired
    public PredictionBasedPrototypeApplication(EmployeeRepository employeeRepository) {
    ...
    public void run(String... strings) {
        LOG.info("Cleaning database of old data");
        employeeRepository.deleteAll();

        LOG.info("Inserting data in DB.");
        employeeList.forEach(employeeRepository::save);
        LOG.info("User count in DB: {}", employeeRepository.count());
    }
}
